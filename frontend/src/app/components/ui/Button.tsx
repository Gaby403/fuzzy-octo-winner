...
function styleFor(variant: Variant): React.CSSProperties {
    ...
    if (variant === "primary")
        return { ...base, padding: "14px 28px", border: "none", background: colors.redBtn, color: colors.pureWhite };
    if (variant === "outline")
        return { ...base, padding: "13px 26px", border: `1px solid ${colors.redInk}`, background: "transparent", color: colors.redInk };
...
export function Button({ variant = "primary", children, arrow, to, href, onClick, type, disabled, ariaLabel, className, style }: BaseProps & {
    ...
    if (to)
        return <Link to={to} onClick={onClick} aria-label={ariaLabel} className={className} style={merged}>{content}</Link>;
    if (href)
        return <a href={href} onClick={onClick} aria-label={ariaLabel} className={className} style={merged} target={/^https?:/.test(href) ? "_blank" : undefined} rel={/^https?:/.test(href) ? "noopener noreferrer" : undefined}>{content}</a>;
