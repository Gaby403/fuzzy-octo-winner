...
export function NewsletterForm({ compact = false }: {
    ...
    return (<form onSubmit={submit} style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
      ...
      <input id={id} type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="voce@email.com" style={{ flex: "1 1 200px", minWidth: 0, background: colors.surface, border: `1px solid ${colors.borderStrong}`, borderRadius: radii.pill, color: colors.white, fontFamily: fonts.body, fontSize: 14, padding: "12px 18px", outline: "none" }}/>
