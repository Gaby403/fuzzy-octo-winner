...
/**
 * Trilha de navegação acessível + JSON-LD BreadcrumbList (SEO).
...
export function Breadcrumbs({ items }: { items: Crumb[] }) {
  ...
  return (
    <nav aria-label="Trilha de navegação" style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.08em", color: "rgba(239,239,239,0.5)" }}>
      <ol style={{ listStyle: "none", display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8, margin: 0, padding: 0 }}>
        {items.map((c, i) => (
          <li key={c.label + i} style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
            {c.to && i < items.length - 1 ? (
              <Link to={c.to} style={{ color: "rgba(239,239,239,0.5)", textDecoration: "none" }}>{c.label}</Link>
            ) : (
              <span aria-current="page" style={{ color: i === items.length - 1 ? RED_INK : "inherit" }}>{c.label}</span>
            )}
