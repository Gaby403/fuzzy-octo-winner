...
/**
 * Rodapé global — renderizado uma vez no Layout, aparece em todas as páginas.
 * Contém: CTA, navegação, contato, redes sociais, newsletter, políticas e
 * direitos autorais. Todo o conteúdo vem do Studio Tabi CMS.
 */
export function Footer() {
  ...
  return (
    <footer id="contato" ref={ref} style={{ backgroundColor: colors.black, fontFamily: fonts.body, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ borderTop: `1px solid ${colors.border}`, borderBottom: `1px solid ${colors.border}`, position: "relative", zIndex: 1 }}>
        <m.div
          ...
          style={{ padding: `clamp(48px,7vw,88px) ${PAGE_PAD}` }}
      ...
      <div style={{ padding: `clamp(48px, 8vw, 88px) ${PAGE_PAD} clamp(32px, 5vw, 56px)`, position: "relative", zIndex: 1 }}>
        <div className="grid grid-cols-1 md:grid-cols-12" style={{ gap: "clamp(36px, 5vw, 56px)" }}>
          ...
          {/* Colunas de links do CMS */}
          {f.columns.map((col, ci) => (
            <div key={col.title + ci} className="md:col-span-2" style={{ minWidth: 0 }}>
          ...
          <div className="md:col-span-2">
            ...
            <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 12 }}>
              {f.social.map((s, si) => (
                <li key={s.label + si}>
                  <a href={s.url || "#"} target={s.url && s.url.startsWith("http") ? "_blank" : undefined} rel={s.url && s.url.startsWith("http") ? "noopener noreferrer" : undefined}
      ...
      <div style={{ borderTop: `1px solid ${colors.border}`, padding: `20px ${PAGE_PAD}`, position: "relative", zIndex: 1 }}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-6">
            {f.legal.map((item, i) => (
              <a key={item.label + i} href={item.url || "#"} onClick={(e) => goTo(item.url, e)} style={{ fontSize: "9px", fontWeight: 500, letterSpacing: "0.10em", color: "rgba(239,239,239,0.3)", textDecoration: "none" }}>{item.label}</a>
            ))}
            {content.pages.map(pg => (
              <Link key={pg.slug} to={`/p/${pg.slug}`} style={{ fontSize: "9px", fontWeight: 500, letterSpacing: "0.10em", color: "rgba(239,239,239,0.3)", textDecoration: "none" }}>{pg.title}</Link>
            ))}
