      ...
      <main id="conteudo" style={{ position: "relative", zIndex: 1, maxWidth: 900, margin: "0 auto", padding: `clamp(44px,7vw,88px) ${pad} 120px` }}>
        {!service ? (<div>
            ...
            <div style={{ marginTop: "clamp(40px,6vw,64px)", paddingTop: "clamp(28px,4vw,40px)", borderTop: "1px solid rgba(239,239,239,0.10)", display: "flex", flexWrap: "wrap", gap: 16, alignItems: "center", justifyContent: "space-between" }}>
              <Link to="/contato" style={{ display: "inline-flex", alignItems: "center", gap: 12, padding: "14px 28px", borderRadius: 999, background: RED_BTN, color: PURE_WHITE, textDecoration: "none", fontSize: "10px", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase" }}>
                Solicitar orçamento <span style={{ fontSize: 14 }}>→</span>
              </Link>
              {next && next.slug && next.slug !== service.slug && (<Link to={`/servicos/${next.slug}`} style={{ color: "rgba(239,239,239,0.6)", textDecoration: "none", fontSize: 11, fontWeight: 600, letterSpacing: "0.13em", textTransform: "uppercase" }}>
                  Próximo: {next.title} →
                </Link>)}
            </div>
          </article>)}
