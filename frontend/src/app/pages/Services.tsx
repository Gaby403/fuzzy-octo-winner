...
export default function Services() {
    ...
    useEffect(() => {
        document.title = `${t("menu.servicos")} — ${content.site.title}`;
        window.scrollTo(0, 0);
      ...
      <main id="conteudo">
        <div style={{ padding: `clamp(48px, 8vw, 96px) ${pad} clamp(28px, 4vw, 48px)` }}>
          ...
          <h1 style={{ fontFamily: FONT_HEAD, fontWeight: 900, fontSize: "clamp(44px, 7vw, 96px)", letterSpacing: "-0.05em", textTransform: "uppercase", margin: 0, lineHeight: 0.92 }}>
            {t("servicos.titulo")} <span style={{ color: RED }}>{t("servicos.destaque")}</span>
          </h1>
        </div>
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" style={{ listStyle: "none", margin: 0, padding: `0 ${pad} clamp(64px, 10vw, 120px)`, gap: "0 clamp(24px, 3vw, 48px)" }}>
          {content.services.map((s, i) => {
            const to = s.slug ? rota("servico", s.slug) : undefined;
            return (<li key={s.num || s.title}>
                <m.a href={to} onClick={to ? (e) => { e.preventDefault(); navigate(to); } : undefined} aria-label={`${t("servico.ver")}: ${s.title}`} style={{ display: "block", textDecoration: "none", color: WHITE, borderTop: "1px solid rgba(239,239,239,0.10)", paddingTop: "clamp(24px,3vw,36px)", paddingBottom: "clamp(24px,3vw,36px)", cursor: to ? "pointer" : "default" }} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.7, delay: (i % 3) * 0.06, ease: [0.16, 1, 0.3, 1] }}>
