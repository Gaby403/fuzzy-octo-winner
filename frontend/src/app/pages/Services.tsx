...
/** Índice de serviços (/servicos) — lista todos os serviços do CMS. */
    ...
    <div style={{ minHeight: "100svh", background: BLACK, color: WHITE, fontFamily: FONT_BODY }}>
      ...
      <main id="conteudo">
        ...
        <ul
          ...
          style={{ listStyle: "none", margin: 0, padding: `0 ${pad} clamp(64px, 10vw, 120px)`, gap: "0 clamp(24px, 3vw, 48px)" }}
        >
          {content.services.map((s, i) => {
            const to = s.slug ? `/servicos/${s.slug}` : undefined
            return (
              <li key={s.num || s.title}>
                <m.a
                  href={to}
                  onClick={to ? (e) => { e.preventDefault(); navigate(to) } : undefined}
                  aria-label={`Ver serviço: ${s.title}`}
                  style={{ display: "block", textDecoration: "none", color: WHITE, borderTop: "1px solid rgba(239,239,239,0.10)", paddingTop: "clamp(24px,3vw,36px)", paddingBottom: "clamp(24px,3vw,36px)", cursor: to ? "pointer" : "default" }}
