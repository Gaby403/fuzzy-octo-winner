...
export default function ThankYou() {
    ...
    return (<div id="conteudo" style={{
      ...
      <m.div style={{ position: "relative", width: "clamp(180px, 34vw, 300px)", height: "clamp(180px, 34vw, 300px)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "clamp(32px, 5vw, 56px)" }} initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 1.1, ease: EASE }}>
        
        <m.div aria-hidden="true" style={{ position: "absolute", inset: "-8%", borderRadius: "50%", background: `radial-gradient(circle, ${RED}44 0%, transparent 68%)` }} animate={{ scale: [1, 1.12, 1], opacity: [0.55, 0.85, 0.55] }} transition={{ duration: 3.4, ease: "easeInOut", repeat: Infinity }}/>
        
        <m.div style={{ position: "absolute", inset: 0, borderRadius: "50%", background: RED }} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 0.9, ease: EASE, delay: 0.05 }}/>
        
        <m.div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "center", justifyContent: "center" }} initial={{ rotate: -14, scale: 0.6, opacity: 0 }} animate={{ rotate: 0, scale: 1, opacity: 1 }} transition={{ duration: 1.2, ease: EASE, delay: 0.25 }}>
          <m.div animate={{ y: ["0%", "-6%", "0%"] }} transition={{ duration: 4.2, ease: "easeInOut", repeat: Infinity }}>
      ...
      <m.h1 style={{ fontFamily: FONT_HEAD, fontWeight: 900, fontSize: "clamp(44px, 8vw, 104px)", letterSpacing: "-0.05em", textTransform: "uppercase", lineHeight: 0.9, margin: 0 }} initial={{ y: 24, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.9, ease: EASE, delay: 0.5 }}>
