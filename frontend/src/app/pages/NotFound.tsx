...
export default function NotFound() {
  ...
  useEffect(() => {
    document.title = `Página não encontrada — ${content.site.title}`
  ...
  return (
    <div style={{ minHeight: "100svh", background: BLACK, color: WHITE, fontFamily: FONT_BODY, display: "flex", flexDirection: "column" }}>
      ...
      <main id="conteudo" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "60px clamp(20px,5vw,32px)", position: "relative", overflow: "hidden" }}>
        ...
        <h1 style={{ fontFamily: FONT_HEAD, fontWeight: 900, fontSize: "clamp(22px,3vw,40px)", textTransform: "uppercase", letterSpacing: "-0.03em", margin: "8px 0 0" }}>Página não encontrada</h1>
        <p style={{ fontSize: "clamp(13px,1vw,16px)", color: "rgba(239,239,239,0.6)", maxWidth: 440, margin: "16px 0 0", lineHeight: 1.7 }}>
          A página que você procura mudou de lugar ou nunca existiu. Toda jornada tem desvios — vamos te levar de volta ao caminho.
        </p>
        <Link to="/" style={{ marginTop: "clamp(28px,4vw,40px)", display: "inline-flex", alignItems: "center", gap: 12, padding: "15px 30px", borderRadius: 999, background: RED_BTN, color: PURE_WHITE, textDecoration: "none", fontSize: "10px", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase" }}>
          Voltar para a home <span style={{ fontSize: 14 }}>→</span>
        </Link>
        <nav aria-label="Links úteis" style={{ marginTop: 28, display: "flex", flexWrap: "wrap", gap: 18, justifyContent: "center" }}>
          {links.map(l => (
            <Link key={l.to} to={l.to} style={{ color: RED_INK, textDecoration: "none", fontSize: 12, fontWeight: 600, letterSpacing: "0.06em" }}>{l.label}</Link>
          ))}
        </nav>
      </main>
    </div>
