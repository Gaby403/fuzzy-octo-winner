...
export default function ProcessStep() {
    ...
    useEffect(() => {
        let alive = true;
        setLoaded(false);
        fetchPage(slug, locale).then(p => { if (alive) {
            setPage(p);
            setLoaded(true);
        } });
        document.title = `${step ? step.title : t("processo.titulo")} — ${content.site.title}`;
        window.scrollTo(0, 0);
        return () => { alive = false; };
    }, [slug, step, content.site.title, locale, t]);
    return (<div ref={pageRef} style={{ minHeight: "100svh", background: colors.black, color: colors.white, fontFamily: fonts.body, position: "relative", overflow: "hidden" }}>
      ...
      <main id="conteudo" style={{ position: "relative", zIndex: 1, maxWidth: 900, margin: "0 auto", padding: `clamp(40px,6vw,72px) ${PAGE_PAD} 120px` }}>
        <Breadcrumbs items={[{ label: t("geral.home"), to: rota("home") }, { label: t("processo.comoTrabalhamos"), to: rota("sobre") }, { label: step ? step.title : slug }]}/>

        {!step ? (<div style={{ paddingTop: 40 }}>
            <h1 style={{ fontFamily: fonts.head, fontWeight: 900, fontSize: "clamp(36px,7vw,64px)", textTransform: "uppercase", margin: "0 0 16px" }}>{t("processo.naoEncontrada")} <span style={{ color: colors.red }}>{t("servico.encontrado")}</span></h1>
            <Link to={rota("sobre")} style={{ color: colors.redInk, textDecoration: "none", fontWeight: 600 }}>{t("processo.verProcesso")}</Link>
          </div>) : (<>
            <m.div style={{ display: "flex", alignItems: "center", gap: 14, margin: "22px 0 18px", color: colors.red }} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: ease.out }}>
              ...
              <span style={{ fontFamily: fonts.body, fontSize: 11, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: colors.redInk }}>{t("processo.etapa")} 0{idx + 1}</span>
            </m.div>
            ...
            {loaded && page && page.content ? (<div className="wp-page-content" style={{ lineHeight: 1.8, fontSize: 17, color: colors.text }} dangerouslySetInnerHTML={{ __html: page.content }}/>) : loaded ? (<p style={{ color: colors.textFaint }}>{t("processo.emBreve")}</p>) : (<div className="stcms-skeleton" style={{ height: 200, borderRadius: 12 }}/>)}
            ...
            <div style={{ marginTop: "clamp(40px,6vw,64px)", paddingTop: "clamp(28px,4vw,40px)", borderTop: `1px solid ${colors.border}`, display: "flex", flexWrap: "wrap", gap: 16, alignItems: "center", justifyContent: "space-between" }}>
              <Link to={rota("contato")} style={{ display: "inline-flex", alignItems: "center", gap: 12, padding: "14px 28px", borderRadius: 999, background: colors.redBtn, color: colors.pureWhite, textDecoration: "none", fontSize: "10px", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase" }}>
                {t("processo.iniciar")} <span style={{ fontSize: 14 }}>→</span>
              </Link>
              <div style={{ display: "flex", gap: 20 }}>
                {prev && <Link to={rota("processo", prev.slug)} style={{ color: colors.textMuted, textDecoration: "none", fontSize: 11, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase" }}>← {prev.title}</Link>}
                {next && <Link to={rota("processo", next.slug)} style={{ color: colors.redInk, textDecoration: "none", fontSize: 11, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase" }}>{next.title} →</Link>}
              </div>
            </div>
          </>)}
      </main>
